...
// Le nom du transfert est passé en paramètre d'URL (?name=...).
// On garde basename() en secours pour d'anciens liens éventuels.
$URL__ = isset($_GET['name']) ? $_GET['name'] : $url->get_basename();
// Sécurité : le nom est un identifiant (timestamp), on retire tout caractère dangereux (anti-injection SQL)
$URL__ = preg_replace('/[^A-Za-z0-9._-]/', '', $URL__);
?>
 ...
 */
 ...
 if (empty($dynamicVariables['name']) || empty($dynamicVariables['file_path'][0])) {
    echo "Aucun fichier " ;
 }
 else {
...
<body>
    ...
    <a href="<?= htmlspecialchars($dynamicVariables['file_path'][0]) ?>" download>Télécharger le fichier</a>
<br/> 
