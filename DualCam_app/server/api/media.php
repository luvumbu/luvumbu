...
$shareTok = isset($_GET['share']) ? (string) $_GET['share'] : '';
if ($shareTok !== '') {
    // --- Accès PUBLIC via lien de partage (aucune connexion requise) ---
    // Ne sert QUE les vidéos DualCam d'un compte dont le partage public est ACTIF.
    $owner = Auth::userIdFromShareToken($shareTok);
    if ($owner === null) { http_response_code(403); exit('Partage indisponible'); }
    $stmt = Db::pdo()->prepare(
        'SELECT user_id, stored_path, deleted_at FROM ' . TBL_PHOTOS .
        " WHERE id = ? AND user_id = ? AND deleted_at IS NULL AND source = 'dualcam'"
    );
    $stmt->execute([$id, $owner]);
} else {
    // --- Accès PRIVÉ : propriétaire connecté, ou admin (voit tout) ---
