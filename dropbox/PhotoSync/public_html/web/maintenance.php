...
/** Chemin physique d'une ligne photo (active => uploads, sinon => corbeille). */
function physicalPath(array $row): string {
    $base = empty($row['deleted_at']) ? UPLOAD_DIR : Photos::trashDir();
...
if (($_POST['action'] ?? '') === 'purge' && !empty($_POST['ids']) && is_array($_POST['ids'])) {
    try {
        ...
        foreach ($_POST['ids'] as $v) {
            ...
            if ($pid <= 0) continue;
            $owner->execute([$pid]);
            $uid = (int) ($owner->fetch(PDO::FETCH_ASSOC)['user_id'] ?? 0);
...
if ($dbError === '') {
    try {
        ...
        foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $totalRows++;
            if (!is_file(physicalPath($row))) $orphans[] = $row;
        }
    } catch (Throwable $e) {
