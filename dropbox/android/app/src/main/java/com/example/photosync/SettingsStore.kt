...
class SettingsStore(context: Context) {
    ...
    // ---- Types de fichiers à synchroniser (décocher = ne pas envoyer ce type) ----

    /** Envoyer les photos (images). */
    var uploadPhotos: Boolean
        get() = prefs.getBoolean("up_photos", true)
        set(v) { prefs.edit().putBoolean("up_photos", v).apply() }

    /** Envoyer les vidéos. */
    var uploadVideos: Boolean
        get() = prefs.getBoolean("up_videos", true)
        set(v) { prefs.edit().putBoolean("up_videos", v).apply() }

    /** Envoyer la musique / l'audio (désactivé par défaut). */
    var uploadAudio: Boolean
        get() = prefs.getBoolean("up_audio", false)
        set(v) { prefs.edit().putBoolean("up_audio", v).apply() }
    ...
    companion object {
        ...
        // Le serveur PHP (api/, uploads/, lib/…) est déposé dans le sous-dossier "dropbox"
        // du domaine. Vide = racine du domaine.
        const val DEFAULT_SUBPATH = "dropbox"
        const val DEFAULT_URL = "https://luvumbu.com/dropbox"
