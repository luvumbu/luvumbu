...
// Deux moteurs : voix native Android (dans l'app) sinon SpeechSynthesis (navigateur).
const nativeTTS = window.AndroidTTS || null;
document.getElementById('listen').addEventListener('click', () => {
  if (!course || (!nativeTTS && !('speechSynthesis' in window))) return;
  ...
  if (nativeTTS) {
    nativeTTS.stop();
    parts.forEach((text, i) => nativeTTS.speak(String(text), 0.9, 1.1, i === 0, 'cours' + i));
    return;
  }
